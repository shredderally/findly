    ...
    return (
      <div className="auth-pg">
        <div className="auth-c">
          ...
          <div className="asw">No account? <a onClick={() => setView('signup')}>Create one</a> · <a onClick={() => setView('landing')}>Back</a></div>
        </div>
      </div>
    );
  }
        ...
        <div className="auth-c">
          ...
          <form onSubmit={e => handleAuth(e, 'signup')}>
            ...
            <input className="field-input" type="email" required
              value={authForm.email} onChange={e => setAuthForm({ ...authForm, email: e.target.value })} />
            <label className="field-label">Password</label>
            <input className="field-input" type="password" required minLength={8}
              value={authForm.password} onChange={e => setAuthForm({ ...authForm, password: e.target.value })} />
            <button className="btn btn-clay" style={{ width: '100%' }} type="submit">Create free account</button>
          </form>
          <div className="asw">Have an account? <a onClick={() => setView('signin')}>Sign in</a> · <a onClick={() => setView('landing')}>Back</a></div>
        </div>
      </div>
    );
  }
