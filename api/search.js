...
export default async function handler(req, res) {
  ...
  try {
    ...
    let query = supabase
      .from('businesses')
      // Contacts free by default.
// Providers with rating >= 4.5 AND review_count >= 3 are "top rated" —
// their contact is gated behind GHS 2. This is computed client-side
// using the rating/review_count fields returned here.
      .select('id, business_name, category, ghpost_gps_address, description, phone, whatsapp, rating, review_count, verification_status, listing_expires_at')
