  ...
  try {
    ...
    if (existingUnlock) {
      ...
      const { data: biz } = await supabase
        ...
        .eq('id', business_id)
        .single();
      return res.status(200).json({ phone: biz.phone, whatsapp: biz.whatsapp, unlock_id: existingUnlock.id });
    }
    ...
    const { data: newUnlock, error: unlockInsertError } = await supabase
      .from('unlocks')
      .insert({
        customer_id: null,
        business_id,
    ...
    return res.status(200).json({ phone: biz.phone, whatsapp: biz.whatsapp, unlock_id: newUnlock.id });
  } catch (err) {
