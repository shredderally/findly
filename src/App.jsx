...
export default function App() {
  ...
  // unlockedContacts: { [business_id]: { phone, whatsapp } }
  ...
  async function completeUnlock(businessId, reference) {
    try {
      ...
      const data = await res.json();
      if (res.ok) {
        setUnlockedContacts(prev => ({ ...prev, [businessId]: { phone: data.phone, whatsapp: data.whatsapp } }));
      }
    } catch {}
  }
    ...
    <div className="app"><Header />
      <div className="hero">
        <div className="teaser-card">
          ...
          <div className="phone">+233 XX XXX XXXX</div>
        ...
        <button className="btn btn-clay" onClick={() => { setStep(1); setView('signup'); }}>
          List your business
        </button>
        <div style={{ marginTop: 12 }}>
          <a style={{ color: 'var(--muted)', fontSize: 14, cursor: 'pointer' }} onClick={() => setView('browse')}>
            Or browse as a customer →
          </a>
        </div>
      </div>
      <Footer />
      ...
      {listings.map(l => {
        ...
        return (
          <div className="listing" key={l.id}>
            ...
            {/* FIX: contact is gated — only shown after payment */}
