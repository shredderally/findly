...
export default async function handler(req, res) {
  ...
  try {
    ...
    const {
      profilePhoto, idDocFront, idDocBack, livenessSelfie, professionalLicense,
      pledgeAccepted,
    } = req.body;
    ...
    if (!idDocFront || !idDocBack || !livenessSelfie) {
      return res.status(400).json({ error: 'ID document (front + back) and a liveness selfie are required.' });
    }
    ...
    const selfiePath = await uploadDoc(business.id, 'liveness-selfie', livenessSelfie);
    const licensePath = professionalLicense ? await uploadDoc(business.id, 'license', professionalLicense) : null;

    await supabase
      .from('businesses')
      .update({
        profile_photo_path: profilePhotoPath,
        id_doc_front_path: idFrontPath,
        id_doc_back_path: idBackPath,
        liveness_selfie_path: selfiePath,
        professional_license_path: licensePath,
        pledge_accepted: true,
        pledge_accepted_at: new Date().toISOString(),
        // verification_status stays 'pending' — a human (you) reviews
        // documents and flips this manually. Never auto-verify.
      })
      .eq('id', business.id);
